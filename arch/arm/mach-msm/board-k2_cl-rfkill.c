...
static void k2_cl_config_bt_on(void)
{
...
static void k2_cl_config_bt_off(void)
{
	gpio_set_value(MSM_BT_REG_ON, 0);
	mdelay(2);
...
static struct rfkill_ops k2_cl_rfkill_ops = {
...
static int k2_cl_rfkill_remove(struct platform_device *dev)
{
...
static struct platform_driver k2_cl_rfkill_driver = {
	.probe = k2_cl_rfkill_probe,
	.remove = k2_cl_rfkill_remove,
	.driver = {
		.name = "k2_cl_rfkill",
		.owner = THIS_MODULE,
	},
...
static int __init k2_cl_rfkill_init(void)
{
	if (!machine_is_k2_cl())
		return 0;

	return platform_driver_register(&k2_cl_rfkill_driver);
}

static void __exit k2_cl_rfkill_exit(void)
{
	platform_driver_unregister(&k2_cl_rfkill_driver);
}

module_init(k2_cl_rfkill_init);
module_exit(k2_cl_rfkill_exit);
MODULE_DESCRIPTION("k2_cl rfkill");
